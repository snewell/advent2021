...
func countSubPath(end *cave, caves *map[string]*cave, visited *[]bool, last *cave, bigLoop *map[string]struct{}, doubled bool) int {
	count := 0
		...
		if candidate == end {
			...
			if candidate.big {
				...
				if !found {
					...
					count += countSubPath(end, caves, visited, candidate, bigLoop, doubled)
					delete(*bigLoop, candidate.name)
				}
			} else {
				if !(*visited)[candidate.id] {
					(*visited)[candidate.id] = true
					count += countSubPath(end, caves, visited, candidate, &map[string]struct{}{}, doubled)
					(*visited)[candidate.id] = false
				} else if !doubled {
					count += countSubPath(end, caves, visited, candidate, &map[string]struct{}{}, true)
				}
			}
		}
	}
...
func countPaths(caves *map[string]*cave) int {
	...
	return countSubPath(end, caves, &visited, start, &map[string]struct{}{}, false)
}
